...
public class CircleProgressView extends View {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int heightWithoutPadding = height - getPaddingBottom() - getPaddingBottom();
...
//        circleWidth = _circleBounds.width();
		...
		return new RectF(_circleBounds.left + (widthDelta * scaleX), _circleBounds.top + (widthDelta * scaleY), _circleBounds.right - (widthDelta * scaleX), _circleBounds.bottom
				- (widthDelta * scaleY));
	...
	private void setupBounds() {
		...
		mCircleBounds = new RectF(mPaddingLeft + mBarWidth / 2, mPaddingTop + mBarWidth / 2, width - mPaddingRight - mBarWidth / 2, height - mPaddingBottom - mBarWidth / 2);
		mInnerCircleBounds = getInnerCircleRect(mCircleBounds);
		mCircleInnerContour = new RectF(mCircleBounds.left + (mRimWidth / 2.0f) + (mContourSize / 2.0f), mCircleBounds.top + (mRimWidth / 2.0f) + (mContourSize / 2.0f), mCircleBounds.right
				- (mRimWidth / 2.0f) - (mContourSize / 2.0f), mCircleBounds.bottom - (mRimWidth / 2.0f) - (mContourSize / 2.0f));
		mCircleOuterContour = new RectF(mCircleBounds.left - (mRimWidth / 2.0f) - (mContourSize / 2.0f), mCircleBounds.top - (mRimWidth / 2.0f) - (mContourSize / 2.0f), mCircleBounds.right
				+ (mRimWidth / 2.0f) + (mContourSize / 2.0f), mCircleBounds.bottom + (mRimWidth / 2.0f) + (mContourSize / 2.0f));

		mFullRadius = (width - mBarWidth) / 2;
	...
	private void drawCircleWithNumber(Canvas canvas, float _degrees) {
		...
		if (mAutoTextSize) { // only re-calc position and size if string length
			...
			if (mTextLength != text.length()) {
				...
				if (mTextLength == 1) {
					mInnerCircleBounds = new RectF(mInnerCircleBounds.left + (mInnerCircleBounds.width() * 0.1f), mInnerCircleBounds.top, mInnerCircleBounds.right
							- (mInnerCircleBounds.width() * 0.1f), mInnerCircleBounds.bottom);
				} else {
					mInnerCircleBounds = getInnerCircleRect(mCircleBounds);
				}
				...
				if (mShowUnit) {
					...
					textRect = new RectF(mInnerCircleBounds.left, mInnerCircleBounds.top, mInnerCircleBounds.right - ((mInnerCircleBounds.width() * (mRelativeUniteSize)) * relativeGap),
		...
		if (mShowUnit) {
			...
			if (update) {
				...
				mUnitBounds = new RectF(mInnerCircleBounds.left + (mInnerCircleBounds.width() * (1 - mRelativeUniteSize) * relativeGap), mInnerCircleBounds.top, mInnerCircleBounds.right,
						mInnerCircleBounds.bottom);
